        ...
        return (
            <Modal
                ...
                <div className="modal-body">
                    <Form attributes={{id: "form_mod_afficheur_get"}}>

                        <InputSelectEditable
                            editable={true}
                            data={this.state.combo}
                            selectedValue={this.state.afficheur_id_get}
                            placeholder={Lang.get('global.afficheur')}
                            attributes={{
                                name: 'afficheur_id_get',
                                label: Lang.get('global.afficheur'),
                                labelCol: 4,
                                selectCol: 6,
                                htmlFor: 'form_mod_afficheur_get',
                                required: true
                            }}
                ...
                <div className="modal-footer">
                    ...
                    <BtnSave
                        form_id="form_mod_afficheur_get"
...
var store = Reflux.createStore({
    ...
    init: function () {
        ...
        this.listenTo(Actions.validation.verify_form_save, function () {
    ...
    handleAfficheur: function (formDom) {
        ...
        var $form = $(formDom);
        var id = $form.find('[name=afficheur_id_get]').val();
    ...
    updateCombos: function (data) {
        if (data.name == "afficheur_id_get") {
            var retour = {afficheur_id_get: data.value};
            this.trigger(retour);
        }
    },
    /**
    ...
    loadInitData: function (parkId, planId, dataForme) {
        ...
        $.ajax({
            type: 'GET',
            url: BASE_URI + 'parking/' + parkId + '/afficheurs_get',
            processData: false,
            contentType: false,
            context: this
        })
